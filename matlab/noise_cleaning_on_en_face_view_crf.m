...
%         
%         selected_pullbacks = [13, 6, 23, 27, 48, 7, 32];
%         selected_segments = [1, 5, 2, 1, 1, 2, 5];
%         
%         if sum(selected_pullbacks == k) ~= 0
%             index = find(selected_pullbacks == k, 1);
%             
%             if segment == selected_segments(index)
%                 a = 1;
%                 figure;                
%                 subplot(2,1,1),imshow(permute(actual_reshape_GW(:,start_frame:stop_frame,:),[2 1 3]));
%                 subplot(2,1,2),imshow(permute(CRF_Color_GW(:,start_frame:stop_frame,:),[2 1 3]));
%                 
%                 imwrite(permute(actual_reshape_GW(:,start_frame+1:stop_frame,:),[2 1 3]), ['Visualization\Pullback' int2str(k) 'Actual.tif']);
%                 imwrite(permute(CRF_Color_GW(:,start_frame+1:stop_frame,:),[2 1 3]), ['Visualization\Pullback' int2str(k) 'Result.tif']);
%                 
%             end
%             
%         end
...
%     end
    ...
    f1_calcium(k) = CM_Ind_Pullback(2,2,k)/(sum(CM_Ind_Pullback(2,:,k)));
    f1_lipid(k) = CM_Ind_Pullback(3,3,k)/(sum(CM_Ind_Pullback(3,:,k)));
...
disp('\n Metrics \n');
[cmatrix, result] = confusion.getMatrix(actual_reshape_index_full, predict_reshape_index_full);
...
fprintf('Mean and std. error of F1 score for fibrocalcific: %f +/- %f \n', nanmean(f1_calcium), nanstd(f1_calcium)/sqrt(numel(f1_calcium) - sum(isnan(f1_calcium))));
fprintf('Mean and std. error of F1 score for fibrolipidic: %f +/- %f \n', nanmean(f1_lipid), nanstd(f1_lipid)/sqrt(numel(f1_lipid) - sum(isnan(f1_lipid))));