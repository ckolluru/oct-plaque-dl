...
folder = 'C:\Users\Chaitanya\Documents\OCT deep learning dataset\Split sample validation\';
dirList = dir('C:\Users\Chaitanya\Documents\OCT deep learning dataset\Split sample validation\TRF*');
raw_file_names = {'TRF-31-0M-1-RCA-PRE.oct'; 'TRF-32-0M-2-LAD-PRE.oct'; 'TRF-46-0M-1-RCA-PRE.oct'};

frame_nums = {412:521; 260:420; 67:100};
...
% 
%         imwrite(uint16(im_pix_shift), [folder '\Pixel Shifted Images\' currentRawFileName(1:end-4) '_' int2str(currentFrame) '_Pixel_Shifted.tif']);
%         
...
%     end
%  
%     save([folder '\Guidewire_positions\' currentRawFileName(1:end-4) '_Guidewire.mat'], 'Guidewire_Positions');
% end
...
%         end
%         
%         imwrite(uint8(label_alines), [folder '\Labels\' currentRawFileName(1:end-4) '_' num2str(currentFrame) '_Labels.tif']);
%     end
%     
% end
    
%% Make the validation matrix 
labelList = dir('C:\Users\Chaitanya\Documents\OCT deep learning dataset\Split sample validation\Labels\*.tif');
pixShiftList = dir('C:\Users\Chaitanya\Documents\OCT deep learning dataset\Split sample validation\Pixel Shifted Images\*.tif');
...
sample_image = imread(['C:\Users\Chaitanya\Documents\OCT deep learning dataset\Split sample validation\Pixel Shifted Images\' pixShiftList(1).name]);

num_alines_arranged_set = [448, 496, 496];
frame_nums_arranged_set = {67:100; 260:420; 412:521};

total_alines = 0;
for k = 1:numel(num_alines_arranged_set)
    total_alines = total_alines + (num_alines_arranged_set(k)* numel(frame_nums_arranged_set{k}));
end
...
for k = 1:numel(labelList)
    ...
    label = imread(['C:\Users\Chaitanya\Documents\OCT deep learning dataset\Split sample validation\Labels\' labelList(k).name]);
    image = imread(['C:\Users\Chaitanya\Documents\OCT deep learning dataset\Split sample validation\Pixel Shifted Images\' pixShiftList(k).name]);
...
if compare_boolean
    ...
    save([folder 'Split_Sample_Validation.mat'], 'ALine_Label_Matrix');
end
