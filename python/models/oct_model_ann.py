...
# Perform classification (ANN)
class myOCTModel(object):

    def __init__(self, aline_depth = 200, result_dir = "/home/cxk340/OCT_DL/results_ann"):
    ...
    def get_model(self):
    	...
    	# Model architecture for A-line classification with ANN
        model = Sequential()
        model.add(Flatten(input_shape=(self.aline_depth, 1)))
    ...
    def kfoldtrain(self):
        ...
        for kfold in np.arange(num_folds):
            ...
            alines_train = alines_train.reshape(alines_train.shape[0], self.aline_depth, 1)
            alines_validation = alines_validation.reshape(alines_validation.shape[0], self.aline_depth, 1)
            ...
            alines_test = alines_test.reshape(alines_test.shape[0], self.aline_depth, 1)
            ...
            if np.array_equal(np.unique(actual), np.unique(predict)) == False:
                ...
                print('Unique classes in actual and predict')
                print(np.unique(actual))
    ...
    def train_with_validation_set(self, train_ml = True, train_split_sample_validation = False):
        ...
        if np.array_equal(np.unique(actual), np.unique(predict)) == False:
            ...
            print('Unique classes in predict and actual')
            print(np.unique(actual))

        # Extra row and column in confusion matrix for guidewire region, removed from results
        C = np.zeros((4,4))
